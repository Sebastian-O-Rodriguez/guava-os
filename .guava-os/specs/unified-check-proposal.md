# Unified Check Flow Proposal: `guava-os check`
...
```bash
cat issues.json | guava-os check
```
...
# CI gate
cat issues.json | guava-os check --strict
# Exit 1 on any violation including warnings

# Pipe to next for full workflow
cat issues.json | guava-os check && cat issues.json | guava-os next
```
