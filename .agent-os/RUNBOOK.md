# Agent OS Operator Runbook

Operational guide for running the Agent OS CLI in the Guava planning/execution loop.
...
`validate` reports V402 warnings. A sub-issue has a label the CLI doesn't recognize as a persona. Check: is it a misspelling? Should it be added to `.agent-os/config.json` under `labels.persona_labels`?
