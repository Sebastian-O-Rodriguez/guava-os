...
Defines how Agent OS launches execution through Gorp.
Spec only — no implementation in Phase 2A.

## Overview

Gorp is the execution substrate. Agent OS generates directives; Gorp materializes them
...
```json
{
  ...
  "report_path": ".agent-os/reports/GUA-17-backend-{timestamp}.json"
}
```
...
```
1. Operator runs: agent-os next < issues.json
...
- **Trigger**: `agent-os next` returns empty directives
...
```
agent-os-{persona}-{issue_id}
```

Example: `agent-os-backend-GUA-17`

## Integration Points

| System | Direction | Data |
|--------|-----------|------|
| Agent OS → Gorp | Directives (stdout or JSON file) |
