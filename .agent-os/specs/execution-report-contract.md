...
- Reports are emitted by the executing agent, not by Agent OS
- Agent OS does NOT generate, validate, or ingest reports in Phase 2A
...
- `correlation_id` links to the directive's `issue_id + persona` pair from `agent-os next`
...
```
.agent-os/reports/{issue_id}-{persona}-{timestamp}.json
```

Example: `.agent-os/reports/GUA-17-backend-2026-05-15T14-30-00Z.json`
...
- Report ingestion/parsing by Agent OS
