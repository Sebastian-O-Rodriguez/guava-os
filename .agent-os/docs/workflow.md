...
This is how Agent OS is used today. Every step involves human action.
...
5. Operator runs: agent-os validate
6. Operator runs: agent-os status
...
Agent OS does not do this. There is no automated promotion today.
...
Builders (agents) claim Todo sub-issues matching their persona, work on them, and submit for QA. This happens outside Agent OS.

### 8. Review

Human checks progress in Linear. When ready for the next batch, return to step 2.

## What Agent OS Does NOT Do in This Workflow
...
Agent OS is a checkpoint tool. It validates and reports. Humans decide and act.
