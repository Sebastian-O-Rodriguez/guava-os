...
Agent OS is a read-only CLI tool that inspects Linear execution graphs and reports queue state, protocol violations, and parent health for Guava's agent-operated projects.
...
JSON piped to stdin
    ↓
agent-os CLI (pure function: data in → report out)
    ↓
...
- **Agent OS CLI** owns validation and reporting — it tells you what the graph looks like, not what to do about it
- **Agents/builders** own code execution within assigned sub-issues

Agent OS does not have authority to change anything. It is an inspector, not a controller.

## Config

Project-specific settings live in `.agent-os/config.json`:
