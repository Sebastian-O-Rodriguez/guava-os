...
Key terms used throughout Agent OS documentation and output.
...
## Linear Concepts (as used by Agent OS)

### Backlog

Linear status for issues that exist but aren't scheduled. Agent OS classifies these as NOT_PROMOTED. Agents must not claim Backlog work.

### Todo

Linear status for issues ready to be claimed. When a sub-issue is in Todo and passes all eligibility conditions, Agent OS classifies it as EXECUTABLE.
