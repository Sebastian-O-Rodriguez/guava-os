...
_The two terminal behaviors every worker closes on: implement (edit, verify, commit, comment, move to In Review) or judge (verdict only, no code edit). Each issue ends in exactly one terminal action._
